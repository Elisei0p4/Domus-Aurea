...
class CartTests(TestCase):
    ...
    def setUp(self):
        self.product1 = ProductFactory(name='Товар 1', base_price=Decimal('100.00'), discount=0)
        self.product2 = ProductFactory(name='Товар 2', base_price=Decimal('200.00'), discount=0)
    ...
    def test_add_product(self):
        ...
        self.assertEqual(len(self.cart), 1) # 1 уникальная позиция
        self.assertEqual(self.cart.get_total_quantity(), 2) # 2 единицы товара
    ...
    def test_add_same_product_increments_quantity(self):
        """Тестирует, что повторное добавление увеличивает количество."""
    ...
    def test_update_quantity(self):
        """Тестирует обновление количества товара в корзине."""
        self.cart.add(product=self.product1, quantity=1)
        self.cart.add(product=self.product1, quantity=5, update_quantity=True)
    ...
    def test_cart_view_ajax_add(self):
        """Тестирует AJAX-добавление товара в корзину через представление."""
        ...
        self.assertEqual(data['cart_count'], 1) # 1 уникальная позиция